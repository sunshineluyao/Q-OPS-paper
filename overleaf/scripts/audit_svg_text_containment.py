#!/usr/bin/env python3
"""Fail when annotated SVG text escapes its declared container.

Annotate a text element with ``data-container="shape-id"`` and optionally
``data-padding="12"``.  The referenced shape must have an ``id`` and be an
...
def audit(svg: Path, inkscape: str, tolerance: float) -> list[dict[str, object]]:
    root = ET.parse(svg).getroot()
    by_id = {element.get("id"): element for element in root.iter() if element.get("id")}
    annotated = [element for element in root.iter() if local_name(element.tag) == "text" and element.get("data-container")]
...
def main() -> int:
    ...
    for svg in svg_files:
        all_records.extend(audit(svg, args.inkscape, args.tolerance))

    if args.require_annotations and not all_records:
        print("FAIL: no data-container annotations found", file=sys.stderr)
        return 2
    ...
    payload = {
        ...
        "annotated_text": len(all_records),
