...
A leakage-free test must freeze a router using pre-execution features and compare Always Classical, Always Quantum, Random, Evidence-Aware, and a hindsight oracle on held-out instances. Coverage $N^{-1}\sum_i\ind[\pi_\tau(z_i)=Q]$, conditional gain, compute/review cost, violations, certification, and fallback should be reported as thresholds vary. The current data cannot support that curve without retrofitting a prospective policy, so Q-OPS reports the test design and limitation rather than inventing points.
